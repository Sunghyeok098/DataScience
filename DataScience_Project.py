...
data_url = 'C:/Users/hays3/git/python/TP/dataSet.csv'
...
data.to_csv('ana_data.csv', index=False, encoding='cp949')

data_url = 'C:/Users/hays3/git/python/ana_data.csv'
...
for train_index, test_index in kf.split(ana_data):
    ...
    print('When K =5, the accuracy for bagged KNN is : ', metrics.accuracy_score(prediction, y_test))

    model = BaggingClassifier(base_estimator=KNeighborsClassifier(n_neighbors=2), random_state=0, n_estimators=10)
    model.fit(x_train, y_train)
    prediction = model.predict(x_test)
    print('When K =10, the accuracy for bagged KNN is : ', metrics.accuracy_score(prediction, y_test))

    model = BaggingClassifier(base_estimator=KNeighborsClassifier(n_neighbors=2), random_state=0, n_estimators=20)
    model.fit(x_train, y_train)
    prediction = model.predict(x_test)
    print('When K =20, the accuracy for bagged KNN is : ', metrics.accuracy_score(prediction, y_test))

    model = BaggingClassifier(base_estimator=KNeighborsClassifier(n_neighbors=2), random_state=0, n_estimators=50)
    model.fit(x_train, y_train)
    prediction = model.predict(x_test)
    print('When K =50, the accuracy for bagged KNN is : ', metrics.accuracy_score(prediction, y_test))

    model = BaggingClassifier(base_estimator=KNeighborsClassifier(n_neighbors=2), random_state=0, n_estimators=100)
    model.fit(x_train, y_train)
    prediction = model.predict(x_test)
    print('When K =100, the accuracy for bagged KNN is : ', metrics.accuracy_score(prediction, y_test))
